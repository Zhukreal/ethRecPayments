...
/**
 * WinTeam tokens vesting contract.
 *
 * According to the WinTeam token distribution structure, there are two parties that should
 ...
 *
 * The WinTeam "Vesting" smart contract should be in place to ensure meeting the token sale commitments.
 *
...
contract WinTokensVesting {
     ...
     * Address of WinToken.
     */
    ERC20TokenInterface public winToken;
     ...
     *
     * @param token Address of WinToken that will be locked on contract.
    ...
    function getAvailableTokensToWithdraw () public view returns (uint256 tokensToSend) {
        ...
        if (tokensUnlockedPercentage >= 100) {
            tokensToSend = winToken.balanceOf(this);
        } else {
    ...
    function setInitialTokensBalance () private {
        initialTokensBalance = winToken.balanceOf(this);
    }
    ...
    function sendTokens (uint256 tokensToSend) private {
        if (tokensToSend > 0) {
            ...
            winToken.transfer(withdrawAddress, tokensToSend);
            // Raising event
