const Storage = artifacts.require("WinTeamStorage");
...
contract("TeamContracts", (accounts) => {
    ...
    function addMemberTestCaseFactory ({ team, agreementWeeks, weeklyRate, memberAccount, singleTermAgreement }) {
        return async function () {
            ...
            try {
                ...
                assert.fail("WinTeam cannot add new team member; " + e);
            }
    ...
    describe("Token checkup", () => {
        ...
        it("Initial multi-mint in token", async function () {
            const balance = +(await token.balanceOf.call(accounts[WinTeam], {
                from: accounts[WinTeam]
            }));
            if (balance >= initialSupply)
                return assert.ok(true);
            await token.multiMint([accounts[WinTeam]], [initialSupply - balance], {
                from: accounts[deployer]
            });
            assert.ok(true);
        });

        it("WinTeam must own all the tokens", async function () {
            const balance = await token.balanceOf.call(accounts[WinTeam], {
                from: accounts[WinTeam]
            });
            assert.equal(+balance, initialSupply, "WinTeam must own initialSupply tokens");
        });
    ...
    describe("Team Creation", () => {
        ...
        it("Must allow WinTeam to create teams", async function () {
        ...
        it("Must allow WinTeam to create second team", async function () {
    ...
    describe("Team balance refill", () => {
        ...
        const factoryRefillBalance = (fromAccount, amount) => async function () {
            ...
            if (fromAccount !== accounts[WinTeam]) { // first, give to fromAccount some tokens
                await token.transfer(fromAccount, amount, {
                    from: accounts[WinTeam]
                });
            }

            if (fromAccount !== accounts[WinTeam]) { // PRE-APPROVAL must be performed for WinTeam account
                try { // Phase 1: allow smart contract to withdrawal teamInitialTokenReward tokens from WinTeam account
        ...
        it(
            "Must allow WinTeam to refill team balance",
            factoryRefillBalance(accounts[WinTeam], teamInitialTokenReward)
        );
        it(
    ...
    describe("Team member adding", () => {
        ...
        it("Must allow WinTeam to add team member", addMemberTestCaseFactory({
            team: 0,
            agreementWeeks: 2,
            weeklyRate: 50,
    ...
    describe("Team member removal", () => {
        ...
        it("Must allow WinTeam to remove team members", async function () {
    ...
    describe("Team member receives their reward", () => {
        ...
        it("WinTeam adds back first team member", addMemberTestCaseFactory({
            team: 0,
    ...
    describe("Contract upgrade", () => {
        ...
        it("Must deploy new contract", async function () {
            ...
            newTeamContracts = await TeamContracts.new(accounts[WinTeam], token.address, storage.address, { from: accounts[WinTeam] });
        ...
        it("Must allow WinTeam to upgrade a contract", async function () {
